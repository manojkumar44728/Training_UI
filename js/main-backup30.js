    ...
    function showTable(tbl, alias, globalFooter) {
        ...
        for (var i = 0; i < table_.length; i++) {
            ...
            for (var j = 0; j < table_[i].length; j++) {
                if (i == 0) {
                    opts = ''
                    for (var k = 0; k < forTable[clicked_table_name].length; k++) {
                        sel = ''
                        if (forTable[clicked_table_name][k] == alias[j]) {
                            sel = 'selected';
                        }
                        opts += '<option value="' + forTable[clicked_table_name][k] + '" ' + sel + '>' + forTable[clicked_table_name][k] + '</option>'
                    }
